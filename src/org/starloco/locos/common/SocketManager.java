...
import org.starloco.locos.other.Guild.GuildMember;
...
public class SocketManager {
    ...
    public static void MULTI_SEND_Af_PACKET(GameClient out, int position, int totalAbo, int totalNonAbo, int button) {
        send(out, "Af" + position + "|" + totalAbo + "|" + totalNonAbo + "|" + button + "|" + Main.serverId);
    }
    ...
    public static void GAME_SEND_MAP_NPCS_QUEST_UPDATE_SPRITE_PACKETS(GameClient out, GameMap Map, Integer npcTemplateId) {
        //Integer npcId = Map.getNpcs().entrySet()
    	Map.getNpcs().entrySet()
                .stream()
                .filter(entry -> entry.getValue().getTemplate().getId() == npcTemplateId)
                .findFirst()
                .ifPresent((entry) -> {
                    final String packetToUpdate = "GX-|".concat(String.valueOf(entry.getKey()));
                    send(out, packetToUpdate);
                }); // By Coding Mestre : [FIX] - Fixed a bug that was causing DC while doing some quests Close #12

                //.orElseThrow(NoSuchElementException::new)
                //.getKey();

        //final String packetToUpdate = "GX-|".concat(String.valueOf(npcId));
        //send(out, packetToUpdate);

    } // By Coding Mestre - [FIX] - Quest icon now properly disappears as soon as the player obtains the quest. Close #1
    ...
    public static void GAME_SEND_ON_FIGHTER_KICK(Fight f, int guid, int team) {
        ...
        for (Fighter F : f.getFighters(team)) {
            if (F.getPersonnage() == null
                    || F.getPersonnage().getGameClient() == null
                    || F.getPersonnage().getId() == guid)
                continue;
    ...
    public static void GAME_SEND_MESSAGE(Player out, String mess) {
        String packet = "cs<font color='#" + Config.getInstance().colorMessage
                + "'>" + mess + "</font>";
        send(out, packet);
    }
    ...
    public static void GAME_SEND_cMK_PACKET_TO_ALL(Player perso, String suffix,
                                                   int guid, String name, String msg) {
        String packet = "cMK" + suffix + "|" + guid + "|" + name + "|" + msg;
        if (perso.getLevel() < 6) {
    ...
    public static void GAME_SEND_CONSOLE_MESSAGE_PACKET(GameClient out,
                                                        String mess) {
    ...
    public static void GAME_SEND_EHP_PACKET(Player out, int templateID) //Packet d'envoie du prix moyen du template (En réponse a un packet EHP)
    {
        ...
        String packet = "EHP" + templateID + "|"
                + World.world.getObjTemplate(templateID).getAvgPrice();
    ...
    public static void GAME_SEND_cMK_PACKET_INCARNAM_CHAT(Player perso,
                                                          String suffix, int guid, String name, String msg) {
        String packet = "cMK" + suffix + "|" + guid + "|" + name + "|" + msg;
    ...
    //v2.7 - Tactical Mode
    public static void sendTacticalTruePacket(GameClient out)
    {
      send(out,"bfT");
    }

    //v2.7 - Tactical Mode
    public static void sendTacticalFalsePacket(GameClient out)
    {
      send(out,"bfF");
    }
